import {authenticateToken} from '../authanticate-jwt.js'
...
describe("Authenticate jwt", () =>{
    ...
    test("given no authorization header, when invalid,then return error 401", async ()=>{
        const request = {
    ...
    test("given no authorization header, when valid,then add user to request", async ()=>{
        const request = {
            headers: {authorization: "invalid"}
        };
        const response = new ResponseMock();
